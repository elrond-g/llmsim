...
class ConsoleReportFormatter(ReportFormatter):
    ...
    def format(self, model_perf: ModelPerformance) -> None:
        ...
        if all_rows:
            ...
            col_widths["op_weight_mem"] = max(len("权重显存/单卡"), 12) + 1
        else:
            col_widths = {
                "name": 20,
                "type": 10,
        ...
        print("┌" + "─" * (total_width - 2) + "┐")
        header_text = (
            f"性能分析报告: {model_perf.model_name} ({model_perf.forward_mode})"
        )
        ...
        if bottleneck:
            ...
            print("├" + "─" * (total_width - 2) + "┤")
            bottleneck_text = (
                f"性能瓶颈: {op_name} (总耗时: {op_perf.total_time:.3f} ms)"
            )
        ...
        print("├" + "─" * (total_width - 2) + "┤")
        ttft_text = (
            f"TTFT: (耗时: {ttft:.3f} ms)"
            if model_perf.forward_mode == "EXTEND"
            else f"TPOT: (耗时: {ttft:.3f} ms)"
        )
        ...
        print("├" + "─" * (total_width - 2) + "┤")
        weight_mem_text = f"权重显存/单卡: {total_weight_mem_single_gpu:.3f} GB"
